...
(function($) {
    ...
    function onStreamError(self, e) {
        console.log('(有问题检索流——允许访问吗?检查设备的分辨率)There has been a problem retrieving the streams - did you allow access? Check Device Resolution', e);
        doCallback(self, "onError", e);
    }
    function onStreamSuccess(self, stream) {
        console.log("(流获取成功)Stream Success");
    ...
    $.FSRTC.prototype.setMute = function(what) {
        ...
        for (var i = 0,
        len = audioTracks.length; i < len; i++) {
            switch (what) {
            case "on":
    ...
    $.FSRTC.prototype.createAnswer = function(params) {
        ...
        console.log("(音频)Audio constraints", mediaParams.audio);
        console.log("(视屏)Video constraints", mediaParams.video);
        if (self.options.useVideo && self.options.localVideo) {
            getUserMedia({
                ...
                onsuccess: function(e) {
                    ...
                    console.log("(音频加载成功)local video ready");
                },
                onerror: function(e) {
                    console.error("(音频加载异常)local video error!");
                }
            });
        }
        getUserMedia({
    ...
    function RTCPeerConnection(options) {
        ...
        if (options.iceServers) {
            ...
            if (tmp && !(typeof(tmp) == "object" && tmp.constructor === Array)) {
                console.warn("(iceServers必须是一个数组,恢复到默认的iceServers)iceServers must be an array, reverting to default ice servers");
                tmp = null;
            }
            iceServers = {
                iceServers: tmp || [STUN]
    ...
    $.verto.confMan = function(verto, params) {
        ...
        function genMainMod(jq) {
            ...
            if (confMan.params.hasVid) {
                for (var j = 0; j < confMan.canvasCount; j++) {
                    ...
                    var vlhtml = "<div id='" + vlayout_id + "'><br>" + "<b>Video Layout Canvas " + (j + 1) + "</b> <select onChange='$.verto.modfuncs.change_video_layout(\"" + vlayout_id + "\", \"" + j + "\")' id='" + vlselect_id + "'></select> " + "<br><br></div>";
                    jq.append(vlhtml);
                }
        ...
        function genControls(jq, rowid) {
            ...
            html += "<button class='ctlbtn' id='" + kick_id + "'>Kick</button>" + "<button class='ctlbtn' id='" + tmute_id + "'>Mute</button>" + "<button class='ctlbtn' id='" + voldn_id + "'>Vol -</button>" + "<button class='ctlbtn' id='" + volup_id + "'>Vol +</button>" + "<button class='ctlbtn' id='" + transfer_id + "'>Transfer</button>";
            if (confMan.params.hasVid) {
        ...
        case $.verto.enum.state.active:
            ...
            if (speaker && speaker !== "any") {
                ...
                setTimeout(function() {
                    ...
                    attachSinkId(videoElement, speaker);
                },
                500);
            }
            break;
    ...
    $.verto.dialog.prototype.indicateRing = function() {
        ...
        if (dialog.verto.ringer) {
        	...
        	console.log(dialog.verto.options.ringFile);
        	console.log("结束");
